  ...
  switch (action.type) {
    case "add_tweet":
      if(state.isStreaming) {
        ...
        console.log('sentiment: ', sentiment.analyze(action.payload.data.text));
        return {
          ...state,
          tweets: [{data: {...action.payload.data, sentiment: sentiment.analyze(action.payload.data.text)}}, ...state.tweets],
          error: null,
          isWaiting: false,
          errors: [],
        };
      } else {
        console.log('muted tweet...');
        return state;
      }
    case "show_error":
    ...
    case "update_streaming":
      console.log('update_streaming: ', action.payload);
      return { ...state, error: null, isStreaming: action.payload };
    default:
      return state;
  }
};

const TweetFeed = ({isStreaming}) => {
  const initialState = {
    tweets: [],
    error: {},
    isWaiting: true,
  ...
  useEffect(() => {
    dispatch({ type: "update_streaming", payload: isStreaming });
  }, [isStreaming]);
